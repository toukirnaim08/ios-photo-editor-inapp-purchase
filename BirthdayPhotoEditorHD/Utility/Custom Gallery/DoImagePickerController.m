...
- (void)viewDidLoad
{
    ...
    _tvAlbumList.frame = CGRectMake(0, 20, self.view.frame.size.width, self.view.frame.size.height);
    ...
    [[NSNotificationCenter defaultCenter] addObserver: self
                                             selector: @selector(handleEnterForeground:)
                                               ...
                                               object: nil];
    //float heightTemp = self.vBottomMenu.frame.size.height;
    ...
    if(IS_IPHONE_X)
    {
        self.vBottomMenu.frame = CGRectMake(0,29, self.vBottomMenu.frame.size.width, self.vBottomMenu.frame.size.height);
        self.cvPhotoList.frame = CGRectMake(0,
                                            83,
                                            self.cvPhotoList.frame.size.width, self.cvPhotoList.frame.size.height);
    }
    else
    {
        self.vBottomMenu.frame = CGRectMake(0, -1, self.vBottomMenu.frame.size.width, self.vBottomMenu.frame.size.height);
        self.cvPhotoList.frame = CGRectMake(0,
                                            50,self.cvPhotoList.frame.size.width, self.cvPhotoList.frame.size.height);
    }
...
- (IBAction)onSelectAlbum:(id)sender
{
    ...
    if(!isSubViewShowing)
    {
        ...
        [UIView animateWithDuration:0.2 animations:^(void) {
            ...
            _tvAlbumList.frame = CGRectMake(0, 120,
                                            _tvAlbumList.frame.size.width, _tvAlbumList.frame.size.height);
